...
# Turn off if HDF5 Framwork is off
ln -sv $WEST_SIM_ROOT/common_files/get_coords.py
sed "s/RAND/$WEST_RAND16/g" $WEST_SIM_ROOT/common_files/md.in > md.in

# Turn on if HDF5 Framework is off
...
$PMEMD -O -i md.in   -p nacl.parm7  -c parent.ncrst \
...
# Scrape coords and output into npy file.
#python get_coords.py
#cat coords.npy > $WEST_TRAJECTORY_RETURN

cp nacl.parm7 $WEST_TRAJECTORY_RETURN
cp seg.nc $WEST_TRAJECTORY_RETURN

cp nacl.parm7 $WEST_RESTART_RETURN
cp seg.ncrst $WEST_RESTART_RETURN/parent.ncrst

cp seg.log $WEST_LOG_RETURN

# Clean up
rm -f $TEMP md.in seg.nfo seg.pdb get_coords.py
